...
func aboutStrings() {
	...
	assert("smith" < "smitht") // i suppose maybe this could be useful.. someday

	bytes := []byte{'a', 'b', 'c'}
	assert(string(bytes) == "abc") // strings can be created from byte-slices

	bytes[0] = 'z'               //todo pausing here.
	assert(string(bytes) == "z") // byte-slices can be mutated, although strings cannot
